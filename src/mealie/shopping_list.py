...
class ShoppingListMixin:
    ...
    def update_shopping_list_item(
        ...
        # Merge updates into current item (preserves recipeReferences etc.)
    ...
    def update_shopping_list_items_bulk(
        ...
        Args:
            items: List of dicts, each must have an "id" key plus fields to update
        ...
        # Fetch current state for each item and merge updates to preserve
        # existing fields (recipeReferences, note, quantity, etc.)
        merged_items = []
        for item in items:
            item_id = item.get("id")
            if not item_id:
            ...
            current_item = self._handle_request("GET", f"/api/households/shopping/items/{item_id}")
            merged_items.append({**current_item, **item})

        return self._handle_request("PUT", "/api/households/shopping/items", json=merged_items)
